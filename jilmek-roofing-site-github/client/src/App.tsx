...
const ASSETS = {
  logo: "/manus-storage/logo_22da56e1.png",
  hero: "/manus-storage/hero-red-roof_78e57ca6.jpeg",
  pinkRoof: "/manus-storage/home-pink-roof_178510ed.jpeg",
  blueRoof: "/manus-storage/home-blue-roof_36d157bc.jpeg",
  colonnade: "/manus-storage/colonnade_cc91d531.jpeg",
  frame: "/manus-storage/construction-frame_e0f41e5a.jpeg",
  greyHouse: "/manus-storage/house-grey_2ed75e58.jpeg",
  redHouse: "/manus-storage/house-red_3d6d39f5.jpeg",
  tallHouse: "/manus-storage/tall-house_60c0ab4a.jpeg",
  roofFrame: "/manus-storage/roof-frame_e50d38a3.jpeg",
  brickHome: "/manus-storage/brick-home_3a6fc288.jpeg",
};
...
function Home() {
  const listingsQuery = trpc.catalog.listings.useQuery();
  const cmsListings = listingsQuery.data?.length ? listingsQuery.data : mockPropertyListings.filter((listing) => listing.isPublished);
  return <>
...
function Services() {
  ...
  const cmsProducts = productsQuery.data?.length ? productsQuery.data : mockRoofingProducts.filter((product) => product.isPublished);
...
function AdminDashboard() {
  ...
  const [localProducts, setLocalProducts] = useState<MockRoofingProduct[]>(mockRoofingProducts);
  const [localListings, setLocalListings] = useState<MockPropertyListing[]>(mockPropertyListings);
  const utils = trpc.useUtils();
  const productsQuery = trpc.adminCatalog.products.useQuery(undefined, { retry: false });
  const listingsQuery = trpc.adminCatalog.listings.useQuery(undefined, { retry: false });
  const createProduct = trpc.adminCatalog.createProduct.useMutation({ onSuccess: async () => { await utils.adminCatalog.products.invalidate(); setProductForm(emptyProduct); setProductId(null); toast.success("Roofing product saved."); } });
  const updateProduct = trpc.adminCatalog.updateProduct.useMutation({ onSuccess: async () => { await utils.adminCatalog.products.invalidate(); setProductForm(emptyProduct); setProductId(null); toast.success("Roofing product updated."); } });
  const deleteProduct = trpc.adminCatalog.deleteProduct.useMutation({ onSuccess: async () => { await utils.adminCatalog.products.invalidate(); toast.success("Roofing product removed."); } });
  const createListing = trpc.adminCatalog.createListing.useMutation({ onSuccess: async () => { await utils.adminCatalog.listings.invalidate(); setListingForm(emptyListing); setListingId(null); toast.success("Property listing saved."); } });
  const updateListing = trpc.adminCatalog.updateListing.useMutation({ onSuccess: async () => { await utils.adminCatalog.listings.invalidate(); setListingForm(emptyListing); setListingId(null); toast.success("Property listing updated."); } });
  const deleteListing = trpc.adminCatalog.deleteListing.useMutation({ onSuccess: async () => { await utils.adminCatalog.listings.invalidate(); toast.success("Property listing removed."); } });

  if (!user || user.role !== "admin") {
    return <DashboardLayout><div className="cms-unauthorized"><ShieldCheck size={34} /><h1>Admin access required</h1><p>Your account is signed in, but it does not have the admin role needed to manage site content.</p><p>Promote the owner account through the database role field, then reload this page.</p></div></DashboardLayout>;
  }

  const productsApiUnavailable = productsQuery.isError || !productsQuery.data?.length;
  const listingsApiUnavailable = listingsQuery.isError || !listingsQuery.data?.length;
  const usingMockData = productsApiUnavailable || listingsApiUnavailable;
  const products = (productsApiUnavailable ? localProducts : productsQuery.data) ?? [];
  const listings = (listingsApiUnavailable ? localListings : listingsQuery.data) ?? [];
  const isSaving = createProduct.isPending || updateProduct.isPending || createListing.isPending || updateListing.isPending;
  const submitProduct = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const input = { ...productForm, imageUrl: productForm.imageUrl || null, imageKey: productForm.imageKey || null }; if (productsApiUnavailable) { const next = { ...input, id: productId ?? Date.now(), imageUrl: input.imageUrl ?? "", imageKey: input.imageKey ?? null }; setLocalProducts((items) => productId ? items.map((item) => item.id === productId ? next : item) : [next, ...items]); setProductForm(emptyProduct); setProductId(null); toast.success("Demo product saved locally."); } else if (productId) updateProduct.mutate({ id: productId, ...input }); else createProduct.mutate(input); };
  const submitListing = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const input = { ...listingForm, priceLabel: listingForm.priceLabel || null, imageUrl: listingForm.imageUrl || null, imageKey: listingForm.imageKey || null }; if (listingsApiUnavailable) { const next = { ...input, id: listingId ?? Date.now(), priceLabel: input.priceLabel ?? null, imageUrl: input.imageUrl ?? "", imageKey: input.imageKey ?? null }; setLocalListings((items) => listingId ? items.map((item) => item.id === listingId ? next : item) : [next, ...items]); setListingForm(emptyListing); setListingId(null); toast.success("Demo listing saved locally."); } else if (listingId) updateListing.mutate({ id: listingId, ...input }); else createListing.mutate(input); };
  ...
  return <DashboardLayout>
    <div className="cms-shell">
      <div className="cms-topbar"><div><p className="cms-kicker">JILMEK content studio</p><h1>Admin dashboard</h1><p>Keep roofing products and property listings current without editing the website code.</p>{usingMockData && <span className="cms-demo-badge">Demo content · local fallback active</span>}</div><Link href="/" className="cms-view-site">View public site <ArrowRight size={15} /></Link></div>
